function convertValueToConcatNumber(value: string): number {
	let str: string = "";
	[...value].forEach((v, i) => {
		const num: number = value.charCodeAt(i);
		if (!isNaN(num)) {
			str += `${num}`;
		}
	});
...
export function compareFn(a: unknown, b: unknown): number {
	...
	if (typeof a === "string" || typeof b === "string") {
		if (a) {
			if (b) {
				const v1: string = a as string;
				const v2: string = b as string;
				// if (/\d/.test(v1) || /\d/.test(v2)) {
				// 	const n1: number = convertValueToConcatNumber(v1);
				// 	const n2: number = convertValueToConcatNumber(v2);
	...
	if (typeof a === "object" || typeof b === "object") {
		if (a instanceof Date || b instanceof Date) {
			if (a) {
				if (b) {
					const v1: number = (a as Date).getTime();
					const v2: number = (b as Date).getTime();
...
const normalizeMap: Array<{ base: string; letters: RegExp }> = [
	{
		base: "¹",
		letters: /(&#185;|[\u00B9])/g,
	},
...
export function normalizeText(value: string | undefined): string | undefined {
	if (value) {
		for (let i = 0; i < normalizeMap.length; i++) {
			value = value.replace(normalizeMap[i].letters, normalizeMap[i].base);
		}
	}
	return value;
}
